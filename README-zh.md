...
本项目旨在为Qt应用提供稳定可控的渲染后端，让文本、路径、动画、Canvas、GPU渲染统一起来。

QtSkia提供了QWidget、QOpenGLWidget、QQuickWindow、QQuickItem等常用Qt渲染组件与skia的对接，可以方便地将skia引入到现有Qt项目中。

后续将承载高性能代码编辑器、图表、绘图、甚至自定义UI组件，在Windows/Linux/Android上复用同一套绘制能力。
...
QtTinaSkia 升级文档入口：`doc/upgrade/README.md`
...
Qt 6.x（当前验证：Qt 6.9.1，MSVC 2022 x64）
...
当前仓库已切换为 **仅 CMake**（qmake 的 `.pro/.pri` 已移除）。
...
<img src="doc/sponsor/weixin.jpg" width="25%" height="25%" /><img src="doc/sponsor/zhifubao.jpg" width="25%" height="25%" />
